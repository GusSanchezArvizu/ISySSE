...
const char *plaintext = "Hello, ChaCha20 with mbedtls!";
...
int mbedtls_chacha20_self_test( int verbose )
{
	const char *plaintext = "Hello, ChaCha20 with mbedtls!";
	    ...
	    unsigned char decrypted[text_len];
	    ...
	    // Set encryption key
	    mbedtls_chacha20_setkey(&ctx, test_keys[0]);

	    // Encrypt plaintext
	    mbedtls_chacha20_starts(&ctx, test_nonces[0], 0);  // 0 is the counter
	    mbedtls_chacha20_update(&ctx, text_len, (const unsigned char *)plaintext, ciphertext);

	    // Reset context before decryption
	    mbedtls_chacha20_free(&ctx);
	    mbedtls_chacha20_init(&ctx);
	    mbedtls_chacha20_setkey(&ctx, test_keys[0]);

	    // Decrypt ciphertext
	    mbedtls_chacha20_starts(&ctx, test_nonces[0], 0); // Same nonce and counter
	    mbedtls_chacha20_update(&ctx, text_len, ciphertext, decrypted);
	    ...
	    mbedtls_printf("\r\n");
	    mbedtls_printf(" Decrypted Text: %s\r\n", decrypted);
