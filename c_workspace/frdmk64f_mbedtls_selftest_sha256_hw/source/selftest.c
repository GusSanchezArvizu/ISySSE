...
#define HASH_SIZE 32  // SHA-256 produces a 32-byte (256-bit) hash
...
int main(int argc, char *argv[])
{
    ...
    const char *message = "Hello, SHA-256 with mbedtls!";
    ...
    // Start the SHA-256 computation (0 = SHA-256, 1 = SHA-224)
    mbedtls_sha256_starts(&ctx, 0);

    // Feed the message into the SHA-256 function
    mbedtls_sha256_update(&ctx, (const unsigned char *)message, strlen(message));

    // Finish and get the final hash
